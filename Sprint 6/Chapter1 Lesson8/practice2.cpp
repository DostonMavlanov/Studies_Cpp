...
// Умный указатель, удаляющий связанный объект при своем разрушении.
// Параметр шаблона T задает тип объекта, на который ссылается указатель 
template <typename T>
class ScopedPtr {
public:
    ...
    // Деструктор. Удаляет объект, на котрый ссылается умный указатель. 
    ~ScopedPtr() {
        delete ptr_;
    }
    ...
    // Прекращает владение объектом, на которой ссылается умный указатель.
    // Возвращает прежнее значение "сырого" указателя и устанавливает ptr_ в null
    T* Release() noexcept {
        T* other_ptr = ptr_;
        ptr_ = nullptr;
        return other_ptr;
    ...
    T& operator*() const {
        if (ptr_ == nullptr){
            throw logic_error("empty ScopedPtr"s);
        }
        return *ptr_;
    }
